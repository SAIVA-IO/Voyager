...
import { ArrowRight, ArrowLeft, Check, Headphones } from "lucide-react";
import { useMountAnimations } from "@/lib/scroll-setup";

const codeExample = `// Clone a voice
const response = await fetch("https://api.saiva.io/v1/voices/clone", {
  method: "POST",
  headers: {
    "Authorization": "Bearer YOUR_API_KEY"
  },
  body: formData // includes audio sample and voice name
});

const { voice_id } = await response.json();`;

const endpoints = [
  { method: "POST", path: "/v1/voices/clone", desc: "Clone a voice from audio" },
  { method: "GET", path: "/v1/voices", desc: "List your cloned voices" },
  { method: "DELETE", path: "/v1/voices/:id", desc: "Delete a cloned voice" },
];

export default function VoiceCloningApiPage() {
  useMountAnimations();
  return (
        ...
        <section className="relative overflow-hidden bg-gradient-to-br from-gray-50 to-gray-100 py-16 md:py-24">
          <div className="container mx-auto px-4 md:px-6">
            <div className="gs-fade-in max-w-3xl">
              ...
              <Badge variant="secondary" className="mb-6"><Headphones className="h-3 w-3 mr-1" /> Voice Cloning API</Badge>
              <h1 className="text-4xl md:text-5xl font-bold tracking-tight mb-6">Voice Cloning API</h1>
              <p className="text-lg text-muted-foreground mb-8 max-w-xl">Clone voices programmatically from audio samples.</p>
              ...
              <div>
                ...
                <div className="bg-[#0d0d0d] rounded-2xl p-4 sm:p-6 font-mono text-xs sm:text-sm overflow-x-auto">
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    <span className="ml-2 text-gray-500">voice-clone.js</span>
                  </div>
              ...
              <div>
                    ...
                    <Card key={ep.path}>
                      <CardContent className="p-4">
                        <div className="flex items-center gap-3">
                          <Badge className={`${ep.method === "DELETE" ? "bg-red-500" : ep.method === "POST" ? "bg-emerald-500" : "bg-blue-500"} text-white`}>{ep.method}</Badge>
                ...
                <div className="mt-6">
                  ...
                  <ul className="space-y-2">
                    {["Instant cloning", "High fidelity", "Voice verification", "API access"].map((f) => (
